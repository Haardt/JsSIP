...
module.exports = class RTCSession extends EventEmitter
{
    ...
    Promise.resolve()
      ...
      .then(() =>
      {
        ...
        this._connectionPromiseQueue = this._connectionPromiseQueue
          .then(() => mediaConnection.setRemoteDescription('offer', e.sdp))
          .catch((error) =>
          {
            request.reply(488);
      ...
      switch (request.method)
      {
        case JsSIP_C.ACK:
          ...
          if (this._late_sdp)
          {
            ...
            //TODO: AH
            this._connectionPromiseQueue = this._connectionPromiseQueue
              .then(() => this._connection.setRemoteDescription('answer', sdp))
              .then(() =>
              {
  ...
  _processInDialogSdpOffer(request)
  {
    ...
    this._connectionPromiseQueue = this._connectionPromiseQueue
      ...
      .then(() =>
      {
        ...
        // TODO: AH
        return this._connection.setRemoteDescription('offer', e.sdp)
          .catch((error) =>
          {
            request.reply(488);
            debugerror('emit "peerconnection:setremotedescriptionfailed" [error:%o]', error);
  ...
  _receiveInviteResponse(response)
  {
    ...
    switch (true)
    {
      ...
      case /^1[0-9]{2}$/.test(response.status_code):
      {
        ...
        // TODO: AH
        this._connectionPromiseQueue = this._connectionPromiseQueue
          .then(() => this._connection.setRemoteDescription('answer', e.sdp))
      ...
      case /^2[0-9]{2}$/.test(response.status_code):
      {
        ...
        this._connectionPromiseQueue = this._connectionPromiseQueue
          ...
          {
            // TODO: AH
            this._connection.setRemoteDescription('answer', e.sdp)
              .then(() =>
              {
  ...
  _sendReinvite(options = {})
  {
    ...
    function onSucceeded(response)
    {
      ...
      // TODO: AH
      this._connectionPromiseQueue = this._connectionPromiseQueue
        .then(() => this._mediaConnection.setRemoteDescription('answer', e.sdp))
        .then(() =>
        {
