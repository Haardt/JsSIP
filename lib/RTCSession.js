/* globals RTCmediaconnection: false, RTCSessionDescription: false */
...
module.exports = class RTCSession extends EventEmitter
{
    ...
    Promise.resolve()
      ...
      .then(() =>
      {
        ...
        this._connectionPromiseQueue = this._connectionPromiseQueue
          .then(() => mediaConnection.setRemoteSdp('offer', e.sdp))
          .catch((error) =>
          {
            request.reply(488);
      ...
      switch (request.method)
      {
        case JsSIP_C.ACK:
          ...
          if (this._late_sdp)
          {
            ...
            this._connectionPromiseQueue = this._connectionPromiseQueue
              .then(() => this._connection.setRemoteSdp('answer', sdp))
              .then(() =>
              {
  ...
  _close()
  {
    ...
    if (this._connection)
    {
      ...
      {
        debugerror('close() | error closing the RTCmediaconnection: %o', error);
      }
    }
  ...
  _processInDialogSdpOffer(request)
  {
    ...
    this._connectionPromiseQueue = this._connectionPromiseQueue
      ...
      .then(() =>
      {
        ...
        return this._connection.setRemoteSdp('offer', e.sdp)
          .catch((error) =>
          {
            request.reply(488);
            debugerror('emit "mediaconnection:setremotesdpfailed" [error:%o]', error);
  ...
  _receiveInviteResponse(response)
  {
    ...
    switch (true)
    {
      ...
      case /^1[0-9]{2}$/.test(response.status_code):
      {
        ...
        this._connectionPromiseQueue = this._connectionPromiseQueue
          .then(() => this._connection.setRemoteSdp('answer', e.sdp))
      ...
      case /^2[0-9]{2}$/.test(response.status_code):
      {
        ...
        this._connectionPromiseQueue = this._connectionPromiseQueue
          ...
          .then(() =>
          {
            this._connection.setRemoteSdp('answer', e.sdp)
              .then(() =>
              {
  ...
  _sendReinvite(options = {})
  {
    ...
    function onSucceeded(response)
    {
      ...
      this._connectionPromiseQueue = this._connectionPromiseQueue
        .then(() => this._mediaConnection.setRemoteSdp('answer', e.sdp))
        .then(() =>
        {
