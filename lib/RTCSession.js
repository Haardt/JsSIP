...
module.exports = class RTCSession extends EventEmitter
{
  ...
  connect(target, options = {}, initCallback)
  {
    ...
    const mediaConnection = this._mediaConnection;
   ...
   * Answer the call.
   */
  answer(options = {}, mediaConnection)
  {
    debug('answer()');
    ...
    Promise.resolve()
      ...
      .then(() =>
      {
        ...
        if (mediaStream) {
          ...
          return mediaConnection.prepareStreams(mediaConstraints)
            .catch((error) =>
            {
              if (this._status === C.STATUS_TERMINATED)
              {
      ...
      .then(() =>
      {
        ...
        this._connectionPromiseQueue = this._connectionPromiseQueue
          .then(() => mediaConnection.setRemoteDescription(offer))
          .catch((error) =>
          {
            request.reply(488);
      ...
      switch (request.method)
      {
        case JsSIP_C.ACK:
          ...
          if (this._late_sdp)
          {
            ...
            this._connectionPromiseQueue = this._connectionPromiseQueue
              .then(() => this._mediaConnection.setRemoteDescription(answer))
              .then(() =>
              {
  ...
  _createRTCConnection(pcConfig, rtcConstraints)
  {
    ...
    this._mediaConnection.onIceConnectionEvent((state) =>
    {
  ...
  _createLocalDescription(type, constraints)
  {
    ...
    return Promise.resolve()
      ...
      .then(() =>
      {
        if (type === 'offer')
        {
          return mediaConnection.receiveSdpOffer(constraints)
            .catch((error) =>
            {
        ...
        {
          return mediaConnection.receiveSdpAnswer(constraints)
            .catch((error) =>
            {
      ...
      .then(() =>
      {
        return mediaConnection.registerIceUpdates(constraints, type, (eventType, event) => {
          if (eventType === 'ready') {
            this._rtcReady = true;
            debug('emit "sdp"');
            this.emit('sdp', event);
  ...
  _sendInitialRequest(mediaConstraints, rtcOfferConstraints, mediaStream)
  {
    ...
    Promise.resolve()
      ...
      .then(() =>
      {
        ...
        if (this._mediaConnection.type === 'BrowserMediaConnection' && mediaStream)
        {
          return mediaStream;
        }
        // Request for user media access.
        else if (this._mediaConnection.type === 'BrowserMediaConnection' && mediaConstraints.audio || mediaConstraints.video)
        {
          return this._mediaConnection.prepareStreams(mediaConstraints)
            .catch((error) =>
            {
              if (this._status === C.STATUS_TERMINATED)
              {
      ...
      .then((stream) =>
      {
        ...
        if (this._mediaConnection.type === 'BrowserMediaConnection') {
          ...
          return this._mediaConnection.getLocalSdp()
        }
      })
      .then((desc) =>
      {
  ...
  _getDTMFRTPSender()
  {
    const sender = this._mediaConnection.getSenders().find((rtpSender) =>
    {
      ...
      case /^2[0-9]{2}$/.test(response.status_code):
      {
        ...
        this._connectionPromiseQueue = this._connectionPromiseQueue
          .then(() =>
          {
            ...
            if (this._mediaConnection.signalingState === 'stable')
            {
              return this._mediaConnection.receiveSdpOffer(this._rtcOfferConstraints)
                .catch((error) =>
                {
          ...
          .then(() =>
          {
            this._mediaConnection.setRemoteDescription(answer)
              .then(() =>
              {
  ...
  {
    const senders = this._mediaConnection.getSenders().filter((sender) =>
    {
