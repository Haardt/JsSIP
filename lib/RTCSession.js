...
const {isMediaConnection} = require("./MediaConnection");
...
module.exports = class RTCSession extends EventEmitter
{
  ...
  constructor(ua, mediaConnection)
  {
    debug('new');
    ...
    {
      throw new Exceptions.NotSupportedError('No valid MediaConnection instance provided.');
    }
    this._id = null;
    this._ua = ua;
    ...
    // The RTCPeerMediaConnection or custom (nodejs) instance (public attribute).
    this._connection = mediaConnection;
  ...
  connect(target, options = {}, initCallback)
  {
    ...
    const mediaConnection = this._connection;
   ...
   * Answer the call.
   */
  answer(mediaConnection, options = {})
  {
    debug('answer()');

    if (!isMediaConnection(mediaConnection)) {
      throw new Exceptions.InvalidMediaConnectionError('Invalid MediaConnection implementation!')
    }
    ...
    Promise.resolve()
      ...
      .then(() =>
      {
        ...
        this._connectionPromiseQueue = this._connectionPromiseQueue
          .then(() => mediaConnection.setRemoteSdp({type: 'offer', sdp: e.sdp}))
          .catch((error) =>
          {
            request.reply(488);
            ...
            debugerror('emit "mediaconnection:setremotesdpfailed" [error:%o]', error);

            this.emit('mediaconnection:setremotesdpfailed', error);

            throw new Error('peerconnection.setRemoteSdp() failed');
          });
      ...
      switch (request.method)
      {
        case JsSIP_C.ACK:
          ...
          if (this._late_sdp)
          {
            ...
            this._connectionPromiseQueue = this._connectionPromiseQueue
              .then(() => this._connection.setRemoteSdp(new RTCSessionDescription({type: 'answer', sdp: e.sdp})))
              .then(() =>
              {
  ...
  _close()
  {
    ...
    if (this._connection)
    {
      ...
      {
        debugerror('close() | error closing the mediaconnection: %o', error);
      }
    }
  ...
  _createLocalDescription(type, constraints)
  {
    ...
    return Promise.resolve()
      ...
      .then(() =>
      {
        if (type === 'offer')
        {
          return mediaConnection.createSdpOffer(constraints)
            .catch((error) =>
            {
              console.log("Can't create offer", error);
              debugerror('emit "mediaconnection:createofferfailed" [error:%o]', error);

              this.emit('mediaconnection:createofferfailed', error);
        ...
        {
          return mediaConnection.createSdpAnswer(constraints)
            .catch((error) =>
            {
              console.log("Can't create answer", error);
              debugerror('emit "mediaconnection:createanswerfailed" [error:%o]', error);

              this.emit('mediaconnection:createanswerfailed', error);
      ...
      {
        this.emit('sdp', localDescription);
        this._rtcReady = true;
  ...
  _processInDialogSdpOffer(request)
  {
    ...
    this._connectionPromiseQueue = this._connectionPromiseQueue
      ...
      .then(() =>
      {
        ...
        return this._connection.setRemoteSdp(new RTCSessionDescription({type: 'offer', sdp: e.sdp}))
          .catch((error) =>
          {
            request.reply(488);
            debugerror('emit "mediaconnection:setremotesdpfailed" [error:%o]', error);
      ...
      .then(() =>
      {
        ...
        return this._createLocalDescription('answer', this._rtcAnswerConstraints)
          ...
          {
            request.reply(500);
            debugerror('emit "mediaconnection:createtelocaldescriptionfailed" [error:%o]', error);
      ...
      case /^2[0-9]{2}$/.test(response.status_code):
      {
        ...
        this._connectionPromiseQueue = this._connectionPromiseQueue
          .then(() =>
          {
            ...
            return this._connection.createSdpOffer(this._rtcOfferConstraints)
              .catch((error) =>
              {
          ...
          .then(() =>
          {
            this._connection.setRemoteSdp(new RTCSessionDescription({type: 'answer', sdp: e.sdp}))
              .then(() =>
              {
  ...
  _sendReinvite(options = {})
  {
    ...
    function onSucceeded(response)
    {
      ...
      this._connectionPromiseQueue = this._connectionPromiseQueue
        .then(() => this._mediaConnection.setRemoteSdp(
  ...
  {
    const tracks = this._connection.getLocalTracks().filter(track =>
    {
