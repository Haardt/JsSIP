      ...
      switch (request.method)
      {
        case JsSIP_C.ACK:
          ...
          if (this._late_sdp)
          {
            ...
            this._connectionPromiseQueue = this._connectionPromiseQueue
              .then(() => this._connection.setRemoteSdp({type: 'answer', sdp: e.sdp}))
              .then(() =>
              {
  ...
  _processInDialogSdpOffer(request)
  {
    ...
    this._connectionPromiseQueue = this._connectionPromiseQueue
      ...
      .then(() =>
      {
        ...
        return this._connection.setRemoteSdp({type: 'offer', sdp: e.sdp})
          .catch((error) =>
          {
            request.reply(488);
            debugerror('emit "mediaconnection:setremotesdpfailed" [error:%o]', error);
    ...
    switch (true)
    {
      ...
      case /^2[0-9]{2}$/.test(response.status_code):
      {
        ...
        this._connectionPromiseQueue = this._connectionPromiseQueue
          ...
          .then(() =>
          {
            this._connection.setRemoteSdp({type: 'answer', sdp: e.sdp})
              .then(() =>
              {
