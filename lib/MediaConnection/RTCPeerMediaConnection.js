...
module.exports = class RTCPeerMediaConnection extends EventEmitter {
    ...
    createSdpOffer(rtcSessionDescription) {

        return this.rtcPeerConnection.signalingState === 'stable' ? this.rtcPeerConnection.createOffer(rtcSessionDescription)
            .then(offer => {
                return this._setLocalSdp(offer);
            }) : Promise.resolve();
    }

    createSdpAnswer(rtcSessionDescription) {

        return this.rtcPeerConnection.createAnswer(rtcSessionDescription).then(offer => {
            this._setLocalSdp(offer)
        });
    }
    ...
    // TODO: used by un/mute functions. How to handle this?
    getSenders()
    {
